...
class Coffeewithme {
    ...
    public function makeOrder($order_id,$first_name,$last_name,$email,$phone,$pincode,$address_line_1,$address_line_2,$city,$state,$country,$createdBy,$createdTs,$lastUpdatedBy,$lastUpdatedTs,$user_id){
        // echo ' Inside insertApplicant';
        $this->db->query('INSERT INTO order_address(id,order_id,user_id, first_name, last_name, phone, email, address_line1, address_line2,'
                        .' city, state, country, pin, created_ts, created_by, last_updates_ts, last_updated_by) '
                        .' VALUES (0,:order_id,:userid,:first_name,:last_name,:phone,:email,:address_line_1,:address_line_2,:city, '
    ...
    public function getProductList(){
        $this->db->query( 'SELECT c.catergory_name,p.name,p.id,p.price,p.quantity,p.number,p.image FROM products p,category c WHERE c.id=p.category_id ');
        $row = $this->db->resultset();
        return $row;  
    }
    ...
    public function orderCount(){
        $this->db->query('SELECT COUNT(*)as count FROM order_address ');
        $row = $this->db->single();
        return $row;
    }

    public function usercount(){
        $this->db->query('SELECT COUNT(*)as count FROM login ');
        $row = $this->db->single();
        return $row;
    }

    public function lowproduct(){
        $this->db->query(' SELECT c.catergory_name,p.name,p.id,p.price,p.quantity,p.number,p.image FROM products p,category c WHERE c.id=p.category_id AND number < 15 ');
        $row = $this->db->resultset();
        return $row; 
    }

    public function getUserDetails(){
        $this->db->query(' SELECT * FROM user_details WHERE 1 = 1 ');
        $row = $this->db->resultset();
        return $row;
    }

    public function getOrderList(){
        $this->db->query('SELECT oa.first_name,oa.last_name,oa.phone,oa.city,o.grand_total,oa.created_ts FROM order_address oa ,`order` o WHERE oa.order_id = o.order_id; ');
        $row = $this->db->resultset();
        return $row;
    }

    public function updateStockValue($product_id,$stock,$lastUpdatedBy,$lastUpdatedTs){
        $this->db->query(' UPDATE products SET number =:stock,last_updated_ts=:lastUpdatedTs,last_updated_by=:lastUpdatedBy WHERE id =:product_id ');
        $this->db->bind(':product_id', $product_id);
    ...
    public function getAllProductByCartId($cartId){
        $this->db->query(' SELECT product_id,number FROM cart_line WHERE cart_id = :cartId ');
        $this->db->bind(':cartId',$cartId);
        $row = $this->db->resultset();
        return $row;
    }

    public function updateProductStock($product_id,$number,$lastUpdatedBy,$lastUpdatedTs){
        $this->db->query(' UPDATE products SET number = number - :number ,last_updated_ts=:lastUpdatedTs,last_updated_by=:lastUpdatedBy WHERE id =:product_id ');
        $this->db->bind(':product_id', $product_id);
        $this->db->bind(':number',$number);
        $this->db->bind(':lastUpdatedBy',$lastUpdatedBy);
        $this->db->bind(':lastUpdatedTs',$lastUpdatedTs);
